...
namespace day07 {
	...
	struct File : FileBase {
		explicit File( size_t size ) : size{ size } {}
	...
	struct Directory : FileBase {
		explicit Directory( std::shared_ptr< Directory > parent ) : parent{ parent } {}
	...
	class Filesystem {
	public:
		explicit Filesystem( std::shared_ptr< Directory > root ) : root{ root } {};
	...
	class Commands {
	public:
		explicit Commands( const std::vector< Command >& commands ) : commands{ commands } {};
